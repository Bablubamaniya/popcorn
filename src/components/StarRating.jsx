...
const {color,defaultRating,size:totalRating,action=()=>{},} =props;
...
//   console.log(tempRating);
  ...
  function callFill (rating,tempRating,index){
    if (hovering){
        ...
        {new Array(totalRating).fill(0).map(function (_, index) {
          return (
            <Star
              ...
              index={index}
              fill={callFill(rating,tempRating,index)}
              key={index}
            />
          );
        })}
      </div>
      <span style={sStyle}>{tempRating}</span>
    </div>
  );
}
